...
void main()
{
    ...
    fs_Nor = normalize(vec4(u_ModelInvTr * vec3(vs_Nor), 0)); // Pass the vertex normals to the fragment shader for interpolation.
                                                             ...
                                                             // the model matrix.
    vec4 modelposition;
    if (false){//length(vs_OffsetInstanced) > 5) {
        modelposition = u_Model * vs_Pos;
    }
    else {
        modelposition = u_Model * (vs_Pos + vec4(vs_OffsetInstanced, 0));   // Temporarily store the transformed vertex positions for use below
    }
